...
// once they have one. If the lookup fails it falls back to the caller alone, so a
// user service hiccup narrows what is shown instead of widening it.
func OwnerScope(c *gin.Context) ([]string, error) {
	...
	return []string{username, partner}, nil
}
